...
export const NODES: SkillNode[] = [
 ...
 <p><b>Quy trình khép kín:</b> (1) Thiết kế redundancy (multi-AZ, ≥2 bản, DB auto-failover) — bỏ SPOF. (2) Bọc mọi lời gọi bằng timeout/retry/circuit breaker/bulkhead. (3) <b>Load test</b> tới điểm gãy + <b>chaos test</b> (giết pod/DB, tăng latency mạng) để xác nhận hệ 'hỏng một phần' chứ không sập. (4) Thiết lập <b>SLO + alert</b> dựa trên p99/error rate/saturation. (5) Runbook cho sự cố.</p>`,
  ...
  {
    id: "DDD-16", tier: 1, xp: 110, prereq: ["FND-00"],
    title: "DDD & Bounded Contexts",
    sum: "Chia service theo miền nghiệp vụ, không theo tầng kỹ thuật.",
    theory: `<p><b>Domain-Driven Design</b> = thiết kế phần mềm bám theo <b>ngôn ngữ & ranh giới nghiệp vụ</b>. <b>Bounded Context</b> = ranh giới trong đó một mô hình + thuật ngữ là nhất quán (vd 'Course' trong Catalog khác 'Course' trong Enrollment).</p>
 <p>Chia microservices <b>theo bounded context</b> (Identity · Catalog · Enrollment · Payment · Progress) — mỗi context <b>sở hữu data riêng, không share DB</b>, giao tiếp qua API/event. Khái niệm cốt lõi: <b>ubiquitous language</b> (ngôn ngữ chung dev↔nghiệp vụ), <b>aggregate</b> (cụm nhất quán giao dịch), <b>context map</b> (quan hệ giữa các context: đối tác, ACL chống rò rỉ mô hình).</p>
 <p>Đây là câu trả lời cho 'cắt microservices ở đâu'. Chia sai (theo tầng kỹ thuật: UserService/EmailService/DBService) → <b>distributed monolith</b> (coupling chặt nhưng lại thêm độ trễ mạng — tệ nhất cả hai).</p>`,
    whenUse: `<p>Khi tách monolith → microservices hoặc thiết kế hệ nhiều đội. Không cần DDD nặng cho CRUD nhỏ / một đội.</p>`,
    pros: ["Ranh giới rõ → service ít coupling, đội tự chủ", "Ngôn ngữ chung giảm hiểu lầm dev↔business", "Định hướng đúng chỗ để cắt service"],
    cons: ["Học phí cao, dễ over-model", "Chia sai context → phải gộp/tách lại rất đau", "Cần hiểu nghiệp vụ sâu, không thuần kỹ thuật"],
    questions: [
      { q: "Bạn định tách 'UserService, EmailService, DatabaseService'. Sai ở đâu?",
        a: "Đây là chia theo <strong>tầng kỹ thuật</strong>, không theo <strong>nghiệp vụ</strong>. Một thao tác nghiệp vụ (vd 'đăng ký khóa học') sẽ phải gọi qua cả 3 service → coupling chặt + độ trễ mạng = <strong>distributed monolith</strong>. Đúng: chia theo bounded context (Enrollment, Catalog, Payment) — mỗi cái tự chứa logic + data của miền đó." },
      { q: "Vì sao mỗi bounded context nên sở hữu DB riêng, không share DB?",
        a: "Share DB tạo <strong>coupling ngầm qua schema</strong>: đổi bảng ở service A làm vỡ service B, và không thể scale/deploy độc lập. Data riêng + giao tiếp qua API/event cho phép mỗi context <strong>thay đổi mô hình, chọn loại DB, và scale độc lập</strong> — đúng tinh thần microservices." },
    ],
    lab: `<span class="tag">Lab</span><p style="margin-top:8px">Vẽ context map cho edtech: liệt kê 5 bounded context, với mỗi cái ghi [aggregate chính | data sở hữu | 1-2 domain event nó phát ra]. Rồi chỉ ra 1 chỗ nếu chia sai sẽ thành distributed monolith.</p>`,
    links: [
  ...
  {
    id: "API-12", tier: 3, xp: 110, prereq: ["GTW-06"],
    title: "API Design & Versioning",
    sum: "REST/gRPC/GraphQL, versioning, pagination, contract backward-compatible.",
    theory: `<p><b>Contract-first:</b> thiết kế hợp đồng API (OpenAPI/proto) trước — client & server làm song song. <b>Chọn kiểu:</b> REST (public, đơn giản) · gRPC (service↔service, hiệu năng, streaming) · GraphQL (client cần query linh hoạt, nhưng phức tạp N+1/caching).</p>
 <p><b>Versioning & tương thích:</b> ưu tiên <b>backward-compatible</b> (thêm field optional, KHÔNG xóa/đổi nghĩa field cũ) → không cần bump version. Chỉ tăng version (URI <code>/v2</code> hoặc header) khi <b>breaking change</b>. <b>Pagination:</b> cursor &gt; offset ở quy mô lớn. Thêm: error format nhất quán, idempotency key cho POST, rate-limit, phân trang, filtering.</p>`,
    whenUse: `<p>Mọi API dùng chung (public hoặc nội bộ nhiều đội). gRPC cho nội bộ cần hiệu năng; REST cho public; GraphQL khi client đa dạng nhu cầu dữ liệu.</p>`,
    pros: ["Contract ổn định → team/client làm song song", "Versioning tránh vỡ client cũ", "gRPC nhanh + type-safe cho nội bộ"],
    cons: ["Giữ nhiều version = gánh bảo trì", "GraphQL dễ dính N+1 + khó cache", "Thiết kế sai contract khó sửa sau khi client đã dùng"],
    questions: [
      { q: "Cần thêm 1 field vào response — có phải bump lên v2 không?",
        a: "<strong>Không</strong>, nếu thêm <strong>field optional</strong> (backward-compatible) — client cũ bỏ qua field mới vẫn chạy. Chỉ bump version khi <strong>breaking</strong>: xóa field, đổi kiểu/ý nghĩa, hoặc thêm field bắt buộc. Nguyên tắc: 'thêm thì được, đổi/xóa thì phải versioning'." },
      { q: "Vì sao cursor pagination tốt hơn offset ở bảng lớn?",
        a: "<code>OFFSET 1000000</code> buộc DB quét & bỏ 1 triệu dòng → chậm dần theo trang; và nếu data thay đổi giữa các trang → lặp/mất bản ghi. <strong>Cursor</strong> (dùng khóa ổn định như id/created_at của phần tử cuối) chỉ seek đúng chỗ → nhanh và ổn định bất kể sâu bao nhiêu." },
    ],
    lab: `<span class="tag">Lab</span><p style="margin-top:8px">Viết OpenAPI cho 3 endpoint course (list/get/enroll). Thiết kế một thay đổi v1→v2 vừa backward-compatible (thêm field) vừa breaking (đổi field) và phân loại. Đổi endpoint list từ offset sang cursor pagination.</p>`,
    links: [
  ...
  {
    id: "MSG-13", tier: 3, xp: 120, prereq: ["DB-02"],
    title: "Message Queue & Async",
    sum: "Kafka/RabbitMQ, at-least-once, transactional outbox, DLQ.",
    theory: `<p>Giao tiếp <b>bất đồng bộ</b> để tách coupling & hấp thụ tải. <b>Queue</b> (RabbitMQ/SQS: 1 message → 1 consumer, xử lý tác vụ) vs <b>Log</b> (Kafka: nhiều consumer đọc lại, replay, event streaming).</p>
 <p><b>Delivery:</b> mặc định <b>at-least-once</b> → message có thể lặp → consumer phải <b>idempotent</b>. <b>Ordering</b> chỉ đảm bảo trong 1 partition. <b>DLQ</b> (dead-letter queue) cho message hỏng để không kẹt hàng đợi. <b>Transactional outbox:</b> ghi data + event trong CÙNG transaction DB (vào bảng outbox), một relay/CDC publish sau → không mất event khi crash. Theo dõi <b>consumer lag</b> (KEDA scale).</p>`,
    whenUse: `<p>Khi cần tách coupling, chịu spike (buffer), fan-out, xử lý nền (email, encode video, cập nhật read model). KHÔNG dùng cho luồng cần trả lời đồng bộ tức thì cho user.</p>`,
    pros: ["Tách coupling + hấp thụ tải đột biến", "Consumer scale độc lập; Kafka cho phép replay", "Chịu lỗi tốt hơn (retry qua queue)"],
    cons: ["Eventual consistency + khó debug (luồng async)", "Phải tự lo idempotency / DLQ / ordering", "Thêm hạ tầng để vận hành"],
    questions: [
      { q: "Consumer nhận cùng 1 message 2 lần (at-least-once). Xử lý thế nào?",
        a: "Làm <strong>consumer idempotent</strong>: dedupe theo message id / khóa nghiệp vụ đã xử lý, hoặc thiết kế thao tác vốn idempotent (upsert thay vì insert). Không được giả định 'mỗi message xử lý đúng 1 lần' — at-least-once là mặc định thực tế." },
      { q: "'Ghi DB rồi publish event' có thể mất event nếu crash giữa 2 bước. Khắc phục?",
        a: "<strong>Transactional outbox:</strong> trong cùng transaction ghi data, ghi luôn event vào bảng <code>outbox</code>. Một tiến trình relay (hoặc CDC như Debezium) đọc outbox và publish sang broker sau. Vì data + event commit nguyên tử, không có cửa sổ mất event. (Ngược lại là pattern 'dual write' — nguồn gốc lỗi mất event.)" },
    ],
    lab: `<span class="tag">Lab · dùng profile kafka</span><p style="margin-top:8px">Bật Kafka trong labs (<code>docker compose --profile kafka up -d kafka</code>). Viết producer phát 'enroll-event' và consumer cập nhật bảng đếm. Gửi trùng message → chứng minh consumer idempotent. Kill consumer giữa chừng, xem consumer lag rồi tự đuổi kịp khi bật lại.</p>`,
    links: [
  ...
  {
    id: "SEC-15", tier: 3, xp: 120, prereq: ["GTW-06"],
    title: "Security & AuthZ",
    sum: "AuthN vs AuthZ, RBAC/ABAC, JWT/OAuth2, secrets, OWASP.",
    theory: `<p><b>Authentication</b> (bạn là ai) khác <b>Authorization</b> (được làm gì). <b>OAuth2/OIDC</b> chuẩn cho đăng nhập/uỷ quyền. <b>JWT</b> (stateless, dễ scale nhưng khó thu hồi → TTL ngắn + refresh token + blocklist) vs <b>session</b> (dễ revoke nhưng cần store chia sẻ như Redis).</p>
 <p><b>Phân quyền:</b> RBAC (theo vai trò) vs ABAC (theo thuộc tính/ngữ cảnh). Nguyên tắc <b>least privilege</b> + <b>deny by default</b>. <b>Secrets</b> để trong Vault/Secrets Manager, KHÔNG hardcode. Mã hóa <b>in-transit (TLS)</b> & <b>at-rest</b>. Nắm <b>OWASP Top 10</b> (Broken Access Control, Injection...). Multi-tenant: cô lập bằng <b>RLS</b> (như Supabase đang dùng trong chính app này).</p>`,
    whenUse: `<p>Mọi hệ có user / dữ liệu nhạy cảm — thiết kế <b>từ ngày đầu</b>, không phải 'thêm sau'. Bảo mật vá muộn luôn đắt và hở.</p>`,
    pros: ["Bảo vệ dữ liệu + đáp ứng compliance", "Least-privilege/RLS giảm blast radius khi bị xâm nhập", "Chuẩn hóa (OAuth2/OIDC) tái dùng được"],
    cons: ["Thêm phức tạp + chút latency", "JWL revoke khó; cấu hình sai = lỗ hổng nghiêm trọng", "Đòi kỷ luật liên tục (patch, xoay khóa)"],
    questions: [
      { q: "JWT vs session — đánh đổi gì, khi nào chọn cái nào?",
        a: "<strong>JWT</strong> stateless → scale ngang dễ (không cần store chung), nhưng <strong>khó thu hồi</strong> trước hạn (dùng TTL ngắn + refresh token + blocklist khi cần). <strong>Session</strong> dễ revoke tức thì nhưng cần store chia sẻ (Redis). Public API/microservices thường chọn JWT; app web truyền thống 1 khối hợp session." },
      { q: "'Broken Access Control' (IDOR) là gì, phòng thế nào?",
        a: "User truy cập tài nguyên <strong>không thuộc về mình</strong> bằng cách đổi id (vd /orders/123 → /orders/124). Phòng: kiểm <strong>authorization ở tầng service theo ngữ cảnh chủ sở hữu</strong> (không tin tham số client), bật <strong>RLS ở DB</strong>, và <strong>deny-by-default</strong>. Đừng bao giờ dựa vào việc 'client không hiển thị nút đó'." },
    ],
    lab: `<span class="tag">Lab</span><p style="margin-top:8px">Trên Supabase của chính app: xác nhận RLS trên bảng <code>user_progress</code> chặn đọc chéo user (đăng nhập 2 tài khoản, thử query nhau). Giải mã 1 JWT (jwt.io) xem claims (sub/role/exp). Liệt kê 3 mục OWASP Top 10 áp dụng cho edtech và cách phòng.</p>`,
    links: [
  ...
  {
    id: "OBS-14", tier: 4, xp: 120, prereq: ["K8S-08"],
    title: "Observability",
    sum: "Metrics/logs/traces, RED/USE, SLI/SLO, alert đúng cách.",
    theory: `<p>Ba trụ: <b>Metrics</b> (số liệu tổng hợp, rẻ — Prometheus), <b>Logs</b> (structured, sự kiện chi tiết — Loki/ELK), <b>Traces</b> (đường đi request xuyên service — OpenTelemetry/Jaeger). Gắn <b>correlation id</b> để nối 3 trụ.</p>
 <p><b>Khung đo:</b> <b>RED</b> (Rate/Errors/Duration) cho service; <b>USE</b> (Utilization/Saturation/Errors) cho tài nguyên. <b>SLI → SLO → error budget</b>: định nghĩa mức dịch vụ và ngân sách lỗi. <b>Alert theo triệu chứng ảnh hưởng user</b> (SLO burn, p99) — KHÔNG alert theo nguyên nhân (CPU 90%) để tránh alert fatigue.</p>`,
    whenUse: `<p>Từ khi có &gt;1 service / lên production. 'Không đo được thì không vận hành được' — đây cũng là nền tảng để đọc kết quả load/chaos test (TST-11, SYN-B).</p>`,
    pros: ["Tìm & khoanh vùng sự cố nhanh (trace chỉ đúng service chậm)", "Biết hệ có đạt SLA không (dữ liệu, không cảm tính)", "Nền tảng cho autoscale theo custom metric"],
    cons: ["Chi phí lưu trữ metric/log lớn", "Alert sai cách → mệt mỏi, bỏ sót cảnh báo thật", "Thêm hạ tầng + instrument code"],
    questions: [
      { q: "Metrics vs logs vs traces — mỗi loại trả lời câu hỏi gì?",
        a: "<strong>Metrics</strong>: 'có vấn đề không & mức độ bao nhiêu' (RPS, p99, error rate — tổng hợp, rẻ, hợp alert). <strong>Logs</strong>: 'chính xác chuyện gì xảy ra tại một sự kiện' (chi tiết, để điều tra). <strong>Traces</strong>: 'một request đi qua những service nào, chậm/lỗi ở đâu' (xuyên service). Dùng metric để phát hiện, trace để khoanh vùng, log để đào chi tiết." },
      { q: "Nên alert khi p99 vượt SLO, hay khi 'CPU 90%'? Vì sao?",
        a: "Alert theo <strong>p99 vượt SLO</strong> — đó là <strong>triệu chứng ảnh hưởng user</strong>. CPU 90% có thể hoàn toàn vô hại (đang chạy hiệu quả). Alert theo nguyên nhân (CPU/RAM) gây <strong>noise</strong> và bỏ sót cái quan trọng. Dùng USE (CPU/saturation) để <em>điều tra</em> sau khi SLO-alert kêu, không để làm điều kiện alert chính." },
    ],
    lab: `<span class="tag">Lab · gắn với #4</span><p style="margin-top:8px">Thêm Prometheus + Grafana vào labs, scrape demo API. Vẽ dashboard RED (Rate/Errors/Duration). Chạy k6 và quan sát p99 realtime. Tạo 1 alert khi p99 &gt; 200ms. So sánh cảm giác 'nhìn số' vs 'đoán mò'.</p>`,
    links: [
  ...
  {
    id: "FIN-17", tier: 4, xp: 110, prereq: ["INF-10"],
    title: "Cost & FinOps",
    sum: "Chi phí là một ràng buộc thiết kế — ước tính & kiểm soát.",
    theory: `<p>Chi phí là <b>ràng buộc kiến trúc</b> ngang hàng latency/độ tin cậy, không phải chuyện của riêng kế toán. Các trục tối ưu: <b>compute</b> (right-sizing, autoscale, spot/reserved), <b>storage tiering</b> (nóng/lạnh), <b>data transfer/egress</b> (thủ phạm ẩn, nhất là video), <b>managed vs self-host</b> (tính tổng chi phí sở hữu gồm cả công kỹ sư).</p>
 <p><b>Unit economics:</b> $/DAU, $/1000 request — biết một user 'tốn' bao nhiêu để định giá & scale có lời. <b>Cost allocation tags</b> + <b>budget alerts</b>. Nhớ: <b>cache/CDN giảm cả latency lẫn chi phí</b> cùng lúc. Cẩn thận tối ưu cost quá sớm làm chậm tốc độ ra sản phẩm.</p>`,
    whenUse: `<p>Khi chọn instance/dịch vụ và trong review định kỳ; đặc biệt sớm với startup ngân sách hẹp. Không tối ưu cost cực đoan khi còn tìm product-market fit.</p>`,
    pros: ["Tránh cháy túi, quyết định có cơ sở tài chính", "Unit economics hỗ trợ định giá & gọi vốn", "Cache/CDN: một mũi tên trúng cả cost lẫn latency"],
    cons: ["Tối ưu cost quá sớm hại tốc độ & đơn giản", "Đo lường/phân bổ chi phí phức tạp", "Dễ mâu thuẫn với mục tiêu độ tin cậy (cắt redundancy để tiết kiệm)"],
    questions: [
      { q: "Egress là gì, vì sao hay 'cháy túi' với edtech nhiều video?",
        a: "Egress = phí chuyển data <strong>RA internet (và cross-AZ)</strong>. Video/ảnh dung lượng lớn × hàng triệu lượt xem → hóa đơn transfer khổng lồ (thường bị bỏ quên khi ước tính). Giảm bằng <strong>CDN</strong>: cache ở edge, phần lớn request không chạm origin → giảm mạnh egress đồng thời giảm latency." },
      { q: "Managed (RDS) đắt hơn self-host Postgres — khi nào vẫn nên chọn managed?",
        a: "Khi <strong>tổng chi phí sở hữu</strong> (công kỹ sư vận hành HA/backup/patch/failover + rủi ro downtime) <strong>lớn hơn</strong> phần chênh giá. Team nhỏ / tải biến động / thiếu chuyên môn DB → managed thường rẻ hơn về tổng thể. Đừng so mỗi giá niêm yết dịch vụ với giá VM." },
    ],
    lab: `<span class="tag">Lab</span><p style="margin-top:8px">Dùng AWS Pricing Calculator ước tính hóa đơn tháng cho edtech 1M DAU: compute + storage + <b>egress</b> (nhớ tính video). Tính ra <b>$/DAU</b>. Chỉ ra khoản tốn nhất và 1 cách giảm (thường là CDN cho egress).</p>`,
    links: [
  ...
  },
  {
    ...
    prereq: ["LNG-01", "DB-02", "CAP-03", "CSH-04", "SHD-05", "GTW-06", "CQR-07", "K8S-08", "FLT-09", "INF-10", "TST-11", "DDD-16", "API-12", "MSG-13", "SEC-15", "OBS-14", "FIN-17", "SYN-A", "SYN-B"],
    boss: true,
